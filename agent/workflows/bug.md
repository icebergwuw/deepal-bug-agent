...
- 已有 Jira 更新原行；新 Jira 追加一行。新增前选取同页最近一条完整正常数据行作为格式/数据验证模板，不得从空白行或分隔行复制；写入必须使用结构化列请求，禁止以剪贴板 HTML/纯文本作为最终载荷。写后回读 A:J，核对 B 公式、E 复选框、列位、行高、富文本链接和筛选边界，并做一次页面视觉检查。
